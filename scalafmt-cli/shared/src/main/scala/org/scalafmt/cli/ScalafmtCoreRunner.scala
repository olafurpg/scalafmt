...
import org.scalafmt.cli.ConcurrentEnrichments._
  ...
  override private[cli] def run(
    ...
    options.scalafmtConfig match {
      ...
      case Configured.Ok(scalafmtConf) =>
        ...
        val parallelInputMethods =
          if (platform.isJVM) inputMethods.par
          else inputMethods
        parallelInputMethods.foreach { inputMethod =>
          val code = handleFile(inputMethod, options, scalafmtConf)
          exitCode.crossGetAndUpdate(new UnaryOperator[ExitCode] {
