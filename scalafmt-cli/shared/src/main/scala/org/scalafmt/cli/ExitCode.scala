...
object ExitCode {
  ...
  private val cache: ju.Map[Int, ExitCode] =
    if (platform.isJVM)
      new ju.concurrent.ConcurrentHashMap[Int, ExitCode]
    else new ju.HashMap[Int, ExitCode]
  ...
  def apply(code: Int): ExitCode = {
    if (cache.containsKey(code)) cache.get(code)
    else {
