...
For macOS and Linux, it's possible to download pre-built native binaries with
instant startup and fast performance for short-lived Scalafmt runs.

```sh
VERSION=@VERSION@
INSTALL_LOCATION=/usr/local/bin/scalafmt-native
curl https://github.com/scalameta/scalafmt/blob/master/bin/install-scalafmt-native.sh | \
